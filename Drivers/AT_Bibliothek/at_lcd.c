...
/*
 *******************************************************************************
 * Erste Seite, Startbildschirm, usw
 *******************************************************************************
 */
// void taskleiste layer V2.0 Seitenname...
...
void at_lcd_state(TM_STMPE811_t* LCD_Config) {
   ...
   if (TM_STMPE811_TouchInRectangle(LCD_Config, 70, 30, 100, 50)) {
      aktuelle_taste = 1;
   } else if (TM_STMPE811_TouchInRectangle(LCD_Config, 70, 100, 100, 50)) {
      aktuelle_taste = 2;
   } else if (TM_STMPE811_TouchInRectangle(LCD_Config, 70, 170, 100, 50)) {
      aktuelle_taste = 3;
   } else if (TM_STMPE811_TouchInRectangle(LCD_Config, 10, 260, 80, 40)) {
      aktuelle_taste = 4;
...
void at_lcd_debug(uint8_t* UART_Aux) {
   ...
   if (aktuelle_Zeile < MAX_ZEILE) {
      ...
      for (int j = 1; j < MAX_ZEILE; j++) {
         memcpy(&debug_buffer[j - 1], &debug_buffer[j], MAX_BUFFER);
         if (aktuelle_seite == 1) {
            TM_LCD_Puts(debug_buffer[j - 1]);
         }
      }
   }
}
