...
/* Exported macro ------------------------------------------------------------*/
#define Speed_Steps_to_Par(steps)    \
   ((uint32_t)(((steps)*67.108864) + \
               0.5)) /* Speed conversion, range 0 to 15625 steps/s */
#define AccDec_Steps_to_Par(steps) \
   ((uint16_t)(                    \
       ((steps)*0.068719476736) +  \
       0.5)) /* Acc/Dec rates conversion, range 14.55 to 59590 steps/s2 */
#define MaxSpd_Steps_to_Par(steps)  \
   ((uint16_t)(((steps)*0.065536) + \
               0.5)) /* Max Speed conversion, range 15.25 to 15610 steps/s */
#define MinSpd_Steps_to_Par(steps)  \
   ((uint16_t)(((steps)*4.194304) + \
               0.5)) /* Min Speed conversion, range 0 to 976.3 steps/s */
#define FSSpd_Steps_to_Par(steps)                                            \
   ((uint16_t)(                                                              \
       (steps)*0.065536)) /* Full Step Speed conversion, range 7.63 to 15625 \
                             steps/s */

#define Tval_Current_to_Par(Tval)                                           \
   ((uint8_t)(((Tval - 31.25) / 31.25) + 0.5)) /* Torque regulation DAC     \
                                                  current conversion, range \
                                                  31.25mA to 4000mA */
#define Tmin_Time_to_Par(Tmin)     \
   ((uint8_t)(((Tmin - 0.5) * 2) + \
              0.5)) /* Minimum time conversion, range 0.5us to 64us */

#if defined(DEBUG)
#define CHECK_REG(reg, result)                                  \
   if (dSPIN_RegsStruct->##reg != dSPIN_Get_Param(dSPIN_##reg)) \
   result |= 1 << ((uint8_t)dSPIN_##reg)
#define PRINT_REG(reg_name, read_reg, diff, reg_name_string)           \
   strcpy(reg_name_string, "          ");                              \
   ...
   printf("%s %s mcu: 0x%-6X, dspin: 0x%-6X\n", diff, reg_name_string, \
          dSPIN_RegsStruct->##reg_name, read_reg)
#endif /* defined(DEBUG) */
